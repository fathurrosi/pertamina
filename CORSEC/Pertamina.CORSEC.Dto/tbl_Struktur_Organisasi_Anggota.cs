
using System;
//using Thunder.Village.DataAccess;
using DataAccessLayer;
namespace Pertamina.CORSEC.Dto
{
    public class tbl_Struktur_Organisasi_Anggota : IDataMapper<tbl_Struktur_Organisasi_Anggota>
    {
        #region tbl_Struktur_Organisasi_Anggota Properties
        public Int32 id { get; set; }
        public string nama { get; set; }
        public string nip { get; set; }
        public string telp { get; set; }
        public string email { get; set; }
        public Int32? jabatan_id { get; set; }
        public string jabatan_nama { get; set; }
        public DateTime? created { get; set; }
        public string created_by { get; set; }
        public DateTime? updated { get; set; }
        public string updated_by { get; set; }
        #endregion    
        public tbl_Struktur_Organisasi_Anggota Map(System.Data.IDataReader reader)
        {
            tbl_Struktur_Organisasi_Anggota obj = new tbl_Struktur_Organisasi_Anggota();   
            obj.id = Convert.ToInt32(reader["id"]);
            obj.nama = reader["nama"] == DBNull.Value ? null : reader["nama"].ToString();
            obj.nip = reader["nip"] == DBNull.Value ? null : reader["nip"].ToString();
            obj.telp = reader["telp"] == DBNull.Value ? null : reader["telp"].ToString();
            obj.email = reader["email"] == DBNull.Value ? null : reader["email"].ToString();
            obj.jabatan_id = reader["jabatan_id"] == DBNull.Value ? (Int32?) null : Convert.ToInt32(reader["jabatan_id"]);
            obj.jabatan_nama = reader["jabatan_nama"] == DBNull.Value ? null : reader["jabatan_nama"].ToString();
            obj.created = reader["created"] == DBNull.Value ? (DateTime?) null : Convert.ToDateTime(reader["created"]);
            obj.created_by = reader["created_by"] == DBNull.Value ? null : reader["created_by"].ToString();
            obj.updated = reader["updated"] == DBNull.Value ? (DateTime?) null : Convert.ToDateTime(reader["updated"]);
            obj.updated_by = reader["updated_by"] == DBNull.Value ? null : reader["updated_by"].ToString();
            return obj;
        }
    }
}