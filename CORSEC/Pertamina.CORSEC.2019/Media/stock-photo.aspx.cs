...
{
    public partial class stock_photo : CORSECPage
    {
...
";
        ...
        void BindingDataPrint_Ad(string judul)
        {
            ...
            List<tbl_Media_File> files = tbl_Media_FileItem.GetDataPaging(pageIndex, pageSizePrint_Ad, ItemID, out totalRows);
            listViewPrint_Ad.DataSource = files;
            listViewPrint_Ad.DataBind();


            string template = @"<img id=""expandedImg"" src=""{0}"" style=""width: 100%""> ";
            tbl_Media_File file = files.FirstOrDefault();
