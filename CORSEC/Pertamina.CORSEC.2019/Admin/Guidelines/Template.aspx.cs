...
namespace Pertamina.CORSEC._2019.Admin.Guidelines
{
    public partial class Template : AuthorizeAdminPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (item != null)
                {
                    ...
                    tbl_File_Template file = tbl_File_TemplateItem.GetByReff(ReferenceTable.tbl_Guidelines_Info.ToString(), item.id.ToString());
                    if (file != null)
                    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            tbl_Guidelines_Info result = null;
            if (!isEdit)
            {
                result = tbl_Guidelines_InfoItem.Insert(item);
            }
            else
            {
                result = tbl_Guidelines_InfoItem.Update(item);
            }
            ...
            if (result != null)
            {
                ...
                if ((fuImportImage.PostedFile != null) && (fuImportImage.PostedFile.ContentLength > 0))
                {
                    ...
                    tbl_File_Template file = tbl_File_TemplateItem.GetByReff(ReferenceTable.tbl_Guidelines_Info.ToString(), result.id.ToString());
                    if (file == null)
                    {
                    ...
                    else
                    {
                        ...
                        file.ref_name = ReferenceTable.tbl_Guidelines_Info.ToString();
                        tbl_File_TemplateItem.Insert(file);
                    }

                    file = tbl_File_TemplateItem.GetByReff(ReferenceTable.tbl_Guidelines_Info.ToString(), item.id.ToString());
                    if (file != null)
                    {
                        imgThumnail.ImageUrl = "data:image/png;base64," + Convert.ToBase64String(file.file_blob);
                    }
