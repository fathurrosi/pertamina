using System;
using System.Web.UI.WebControls;
using Pertamina.CORSEC.Business;
using Pertamina.CORSEC.Business.Enum;
using Pertamina.CORSEC.Dta;

namespace Pertamina.CORSEC._2019.Admin.SpeechReport
{
    public partial class BoardSpeech : AuthorizeAdminPage
    {
        //public string PrevUrl { get; set; }
        protected void btnCreate_Click(object sender, EventArgs e)
        {
            string selectedDocType = string.Format("{0}", (int)Speech_Report_Type.BoardSpeech);
            Response.Redirect(ResolveUrl(string.Format("~/Admin/SpeechReport/Details/BoardSpeech.aspx{0}&tp={1}", PrevUrl, selectedDocType)));
        }

        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                //ddlTipe.DataTextField = "name";
                //ddlTipe.DataValueField = "name";
                //ddlTipe.DataSource = Utilities.;
                //ddlTipe.DataBind();

                //ddlTipe.DataTextField = "Text";
                //ddlTipe.DataValueField = "Code";
                //ddlTipe.DataSource = Utilities.GetDataSource<Business.Enum.Speech_Report_Type>();
                //ddlTipe.DataBind();
            }

        }

        protected void grid_RowDataBound(Object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.Header)
            {
                //add the thead and tbody section programatically
                e.Row.TableSection = TableRowSection.TableHeader;
            }
            else if (e.Row.RowType == DataControlRowType.DataRow)
            {
                // Display the company name in italics.
                //e.Row.Cells[1].Text = "<i>" + e.Row.Cells[1].Text + "</i>";
                HiddenField hiddenField = e.Row.FindControl("hdn") as HiddenField;
                //HyperLink hyperLink = e.Row.FindControl("hl") as HyperLink;
                //hyperLink.NavigateUrl = string.Format("~/Admin/Details/Question.aspx{0}&id={1}&t=e", PrevUrl, hiddenField.Value);

                HyperLink hlEdit = e.Row.FindControl("hlEdit") as HyperLink;
                hlEdit.NavigateUrl = string.Format("~/Admin/SpeechReport/Details/BoardSpeech.aspx{0}&id={1}&t=e", PrevUrl, hiddenField.Value);

                LinkButton lbtn = e.Row.FindControl("lbDel") as LinkButton;
                lbtn.OnClientClick = "return confirm('Anda yakin akan menghapus item ini?');";
                lbtn.CommandArgument = hiddenField.Value;

                HiddenField hdnFileID = e.Row.FindControl("hdnFileID") as HiddenField;

                HyperLink linkFile = e.Row.FindControl("linkFile") as HyperLink;
                if (!string.IsNullOrEmpty(hdnFileID.Value))
                {
                    linkFile.Text = "Download";
                    linkFile.NavigateUrl = ResolveUrl(string.Format("~/SpeechReportHandler.ashx?FileID={0}", hdnFileID.Value));
                }
            }
        }

        protected void lb_Click(object sender, EventArgs e)
        {
            LinkButton lbtn = sender as LinkButton;
            int _id = 0;
            int.TryParse(lbtn.CommandArgument, out _id);
            tbl_Board_Speech_PresentationItem.Delete(_id);

            grid.DataBind();
        }

        protected void ddlTipe_SelectedIndexChanged(object sender, EventArgs e)
        {
            grid.DataBind();
        }
    }
}