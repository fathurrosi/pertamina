...
{
    public partial class BoardSpeech : AuthorizeAdminPage
    {
        public string Tipe
        {
            get
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (item != null)
                {
                    ...
                    tbl_Board_Speech_Presentation_File file_image = tbl_Board_Speech_Presentation_FileItem.GetByReff(ReferenceTable.tbl_Board_Speech_Presentation_Image.ToString(), item.id.ToString());
                    if (file_image != null)
                    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (result != null)
            {
                ...
                if ((fuImportImage.PostedFile != null) && (fuImportImage.PostedFile.ContentLength > 0))
                {
                    ...
                    tbl_Board_Speech_Presentation_File file = tbl_Board_Speech_Presentation_FileItem.GetByReff(ReferenceTable.tbl_Board_Speech_Presentation_Image.ToString(), result.id.ToString());
                    if (file == null)
                    {
                        file = new tbl_Board_Speech_Presentation_File();
                    }
                    ...
                    else
                    {
                        file.file_id = Guid.NewGuid().ToString();
                        file.ref_id = result.id.ToString();
                        file.ref_name = ReferenceTable.tbl_Board_Speech_Presentation_Image.ToString();
                        tbl_Board_Speech_Presentation_FileItem.Insert(file);
                    }
                }

                Response.Redirect(ResolveUrl(string.Format("~/Admin/SpeechReport/BoardSpeech.aspx{0}", PrevUrl)));
            }
        }
