...
{
    public partial class ks_template : AuthorizeAdminPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (item != null)
                {
                    ...
                    {
                        imgThumnail.ImageUrl = Utilities.ByteToString(file.file_blob);
                    }
                }
            }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                isEdit = false;
                item = new tbl_Kinerja_Sekper_Info();
            ...
            tbl_Kinerja_Sekper_Info result = null;
            if (!isEdit)
            {
                result = tbl_Kinerja_Sekper_InfoItem.Insert(item);
            }
            else
            {
                result = tbl_Kinerja_Sekper_InfoItem.Update(item);
            }
            ...
            if (result != null)
            {
                ...
                if ((fuImportImage.PostedFile != null) && (fuImportImage.PostedFile.ContentLength > 0))
                {
                    ...
                    tbl_File_Template file = tbl_File_TemplateItem.GetByReff(ReferenceTable.tbl_Kinerja_Sekper_Info.ToString(), result.id.ToString());
                    if (file == null)
                    {
                    ...
                    else
                    {
                        ...
                        file.ref_name = ReferenceTable.tbl_Kinerja_Sekper_Info.ToString();
                        tbl_File_TemplateItem.Insert(file);
                    }

                    file = tbl_File_TemplateItem.GetByReff(ReferenceTable.tbl_Kinerja_Sekper_Info.ToString(), item.id.ToString());
                    if (file != null)
                    {
