...
{
    public partial class logo_add : AuthorizeAdminPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                tbl_brand_guideline_logo item = tbl_brand_guideline_logoItem.GetByPK(ItemID);
                if (item != null)
                {
        ...
        {
            tbl_brand_guideline_logo file = tbl_brand_guideline_logoItem.GetByPK(ItemID);
            if (file != null) tbl_brand_guideline_logoItem.Delete(file.id);
            Response.Redirect(ResolveUrl(string.Format("~/Admin/Brand/logos.aspx{0}&id={1}", PrevUrl, ParentItemID)));
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if ((fuImportImage.PostedFile != null) && (fuImportImage.PostedFile.ContentLength > 0))
            {
                ...
                {
                    newFile = true;
                    file = new tbl_brand_guideline_logo();
                ...
                if (!newFile) { tbl_brand_guideline_logoItem.Update(file); }
                else { tbl_brand_guideline_logoItem.Insert(file); }
            }
            Response.Redirect(ResolveUrl(string.Format("~/Admin/Brand/logos.aspx{0}&id={1}", PrevUrl, ParentItemID)));
