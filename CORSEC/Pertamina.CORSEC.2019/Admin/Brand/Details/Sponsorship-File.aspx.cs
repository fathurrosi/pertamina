...
{
    public partial class Sponsorship_File : AuthorizeAdminPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (item != null)
                {
                    ...
                    fileUploaded.NavigateUrl = ResolveUrl(string.Format("~/SponsorshipFileHandler.ashx?id={0}", item.id));
                }
            }
        }
        ...
        {
            tbl_brand_Sponsorship parent = tbl_brand_SponsorshipItem.GetByPK(ParentItemID);
            if (parent != null)
            {
                bool isEdit = true;
                ...
                {
                    isEdit = false;
                    item = new tbl_brand_Sponsorship_File();
                    item.created = DateTime.Now;
                    item.created_by = username;
                    item.sponsorship_id = ParentItemID;
                    item.sponsorship_type = (int)SponsorshipType.Materi_Poster;
                }
                ...
                if (!isEdit) { tbl_brand_Sponsorship_FileItem.Insert(item); }
                else { tbl_brand_Sponsorship_FileItem.Update(item); }
