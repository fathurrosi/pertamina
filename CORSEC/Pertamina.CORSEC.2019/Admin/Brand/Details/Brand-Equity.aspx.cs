...
{
    public partial class Brand_Equity : AuthorizeAdminPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                tbl_brand_equity item = tbl_brand_equityItem.GetByPK(ItemID);
                if (item != null)
                {
                    ...
                    //tbl_File_Program file = tbl_File_ProgramItem.GetByReff(ReferenceTable.tbl_brand_equity.ToString(), item.id.ToString());
                    //if (file != null)
                    //{
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            tbl_brand_equity result = null;
            if (!isEdit)
            {
                result = tbl_brand_equityItem.Insert(item);
            }
            else
            {
                result = tbl_brand_equityItem.Update(item);
            }
            ...
            if (result != null)
            {
                ...
                //    tbl_File_Program file = tbl_File_ProgramItem.GetByReff(ReferenceTable.tbl_brand_equity.ToString(), result.id.ToString());
                //    if (file == null)
                //    {
                ...
                //        file.ref_name = ReferenceTable.tbl_brand_equity.ToString();
                //        tbl_File_ProgramItem.Insert(file);
                //    }

                //    file = tbl_File_ProgramItem.GetByReff(ReferenceTable.tbl_brand_equity.ToString(), item.id.ToString());
                //    if (file != null)
                //    {
                //        imgThumnail.ImageUrl = "data:image/png;base64," + Convert.ToBase64String(file.file_blob);
                //    }
                ...
                //else if (TipeProgram == Tipe_Program.Stakeholder_Relation)
                    Response.Redirect(ResolveUrl(string.Format("~/Admin/Brand/Brand-Equity.aspx{0}", PrevUrl)));
            }
        }
    }
}