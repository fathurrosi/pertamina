...
{
    public partial class logo_guidance : AuthorizeAdminPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                tbl_brand_guideline_user_manual item = tbl_brand_guideline_user_manualItem.GetByPK(ItemID);
                if (item != null)
                {
                    ...
                    fileUploaded.NavigateUrl = ResolveUrl(string.Format("~/LogoGuidanceHandler.ashx?id={0}", item.id));
                }
            }
        }
        ...
        {
            tbl_brand_guideline_user_manual file = tbl_brand_guideline_user_manualItem.GetByPK(ItemID);
            if (file != null) tbl_brand_guideline_user_manualItem.Delete(file.id);
            Response.Redirect(ResolveUrl(string.Format("~/Admin/Brand/logo-guidance.aspx{0}&id={1}", PrevUrl, ParentItemID)));
        }
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                newFile = true;
                item = new tbl_brand_guideline_user_manual();
            ...
            if (!newFile) { tbl_brand_guideline_user_manualItem.Update(item); }
            else { tbl_brand_guideline_user_manualItem.Insert(item); }
            Response.Redirect(ResolveUrl(string.Format("~/Admin/Brand/logo-guidance.aspx{0}&id={1}", PrevUrl, ParentItemID)));
